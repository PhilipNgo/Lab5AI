...
public class QLearningController extends Controller {
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			String new_state_state = StateAndReward.getStateAngle(angle.getValue(), vx.getValue(), vy.getValue());
			String new_state_hover = StateAndReward.getStateHover(angle.getValue(), vx.getValue(), vy.getValue());
			String new_state = new_state_state + new_state_hover;
			...
			double previous_reward = StateAndReward.getRewardAngle(previous_angle, previous_vx, previous_vy) + StateAndReward.getRewardHover(previous_angle, previous_vx, previous_vy);
			action_counter = 0;
			...
			if (previous_state != null) {
				...
				double currAlpha = alpha(Ntable.get(prev_stateaction));
				
				// Gamma is the discount factor and alpha is the learning rate.
				Qtable.put(prev_stateaction, (1.0 - currAlpha) * Qtable.get(prev_stateaction) + currAlpha * (previous_reward + GAMMA_DISCOUNT_FACTOR * getMaxActionQValue(new_state))); 
